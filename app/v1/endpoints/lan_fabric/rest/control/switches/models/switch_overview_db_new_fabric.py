...
class SwitchOverviewResponse:
    ...
    def response_model(self) -> SwitchOverviewResponseModel:
        """
        Returns the current switch overview data for self.fabric as a model.
        """
        if not self._refreshed:
            raise ValueError("Data not refreshed. Call refresh() first.")

        return SwitchOverviewResponseModel(
            switchConfig=self.sync.response_model(),
            switchHealth=self.health.response_model(),
            switchHWVersions=self.build_hw_response(),
            switchRoles=self.roles.response_model(),
            switchSWVersions=self.build_sw_response(),
        )
    ...
    def response_json(self) -> str:
        ...
        """
        return self.response_model().model_dump_json(exclude={"fabric"})
...
def print_roles():
    ...
    with Session(engine) as db_session:
        ...
        roles_record = roles.response_model()
        print("Roles: Model access")
        print(f"  fabric: {roles_record.fabric}")
        print(f"  leaf:   {roles_record.leaf}")
        print(f"  spine:  {roles_record.spine}")
...
def print_switch_overview_response():
    """
...
def main():
    ...
    print_sw()
    print_sync()
    print_switch_overview_response()
